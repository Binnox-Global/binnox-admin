...
export const UserOverviewChart = ({ data = defaultData }: UserOverviewChartProps) => {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const amount = currency === 'NGN' ? convertToNaira(data.amount) : data.amount;
      return (
        <div className="bg-black text-white px-6 py-4 text-[15px] rounded-[10px]">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-[#F46702]" />
            <span className="text-white">Amount : {currency === 'NGN' ? '₦' : '$'}{formatNumber(amount)}</span>
          </div>
        </div>
      );
    }
